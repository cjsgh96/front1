...
*/
$(function(){//window.onload=function(){} 초기화작업
    ...
    let fontSizeField=document.getElementById("fontSize");//폰트 크기

    //맨처음에 로그인했을때 기본 배경색상,글자크기
    let defaultBackground="#ffffff";
    let defaultFontSize=15;

    //1.storage에 저장
    $('#save').click(function(){ //save.onclick=function(){}
       //로컬스토러지.setItem("키명",저장할값)<->로컬스토러지.getItem("키명")<->setItem(키명,저장할값)
    ...
    $('#remove').click(function(){ //remove.onclick=function(){}
        ...
        //형식2) 로컬스토러지 객체명.removeItem(key명) 
